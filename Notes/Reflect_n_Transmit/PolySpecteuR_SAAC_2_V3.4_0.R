PolySpecteuR_SAAC_2<-function(){
  ...
  #   et celui des données interpolées par upolyn I.
  ...
  # On définit le répertoire du script en cours
  #-----------------------------------------
  ...
  # Recherche dans le répertoire, les instruments disponibles et propose le choix
  #-----------------------------------------
  ...
  #-----------------------------------------
  # Recherche les fichiers de paramètres pour chaque instrument sélectionné
  #-----------------------------------------
  ...
  #-----------------------------------------
  # Crée des environnements pour chaque instrument sélectionné et faire une liste
  #-----------------------------------------
  lesenv <- list()
  for (k in 1:N_instruments){
    lesenv[[k]] <- new.env()
  ...
  #-----------------------------------------
  #___________________________________________________________________________
  # Définir les spectros et les plages et intervalles de
 ...
 #___________________________________________________________________________
  ...
  #************************************************************************************
  #Vérification de l'état d'instrument - Fluo - Calcul du facteur de normalisation
  #************************************************************************************
  #-----------------------------------------
  #
  ...
  #***********************************************************************************
  #-----------------------------------------
  #Gère la lampe blanche
  # dum <- readline(paste0("Vérifier la lampe blanche pour tous les instruments? ",
  #                        "? (O)ui/(N)on: \n"))
  # if (toupper(dum)=="N"){
  #   Abort(lesDevices,lesspectros,OOobj,op)
  #   graphics.off()
  ...
  # On démarre la séquence d'acquistion:
  ...
  #On demande le répertoire pour stocker les données.
  #-----------------------------------------
  ...
  while (!DONE){   #Boucle principale sur les échantillons
    ...
    #*****************************************************************************
    #Choisir la prochaine mesure dans le plan d'expérience
    if (selected<nrow){
      ydata=select.list(liste_ech,title="Choisir le prochain échantillon",preselect = liste_ech[selected+1],
                        graphics = TRUE)
    }else
    {
      ydata=select.list(liste_ech,title="Choisir le prochain échantillon",preselect = liste_ech[selected],
                        graphics = TRUE)
    }
    if (ydata==""){
      DONE=TRUE
...
Mesures_Etalon_Fluo <- function(instrument.env,lespectro,OOobj,lesDevices,gainMaya,logo,op,lesEXs){
  ...
  for (k in 1:N_ExFluo){
    ...
    }else
    {
      ...
      barplot(dat_4_plot,names.arg=names_4_plot,main=paste("Excitation à ",
             as.character(lesEXs[k])," nm - Rel. à moyenne",sep=""))
    }
  }
